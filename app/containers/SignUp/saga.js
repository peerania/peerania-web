...
export function* idontHaveEosAccountWorker({ val }) {
  try {
    ...
    const translations = translationMessages[locale];
    const email = yield select(selectEmail());
    const isFacebook = yield select(isFacebookSignUpInitiatedSelector());
    const encryptionKey = isFacebook
      ? window.FB.getAccessToken()
      : yield select(selectEncryptionKey());

    const props = {
      email: isFacebook ? window.FB.getUserID() : email,
      keys,
      masterKey: val[MASTER_KEY_FIELD],
      password: isFacebook ? window.FB.getAccessToken() : val[PASSWORD_FIELD],
